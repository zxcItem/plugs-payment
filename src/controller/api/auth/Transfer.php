...
class Transfer extends Auth
{
    ...
    public function add()
    {
        ...
        // 检查可提现余额
        $balance = BalanceService::recount($this->unid);
        ...
        if (PaymentTransfer::mk()->save($data)) {
            BalanceService::create($this->unid,$data['code'],'用户余额提现',-floatval($data['amount']),'待审核通过后解除锁定',false);
            $this->success('提现申请成功');
        } else {
            $this->error('提现申请失败');
        }
    ...
    public function cancel()
    {
        ...
        BalanceService::cancel($data['code']);
        BalanceService::recount($this->unid);
        $this->success('取消提现成功');
    }
    ...
    public function confirm()
    {
        ...
        BalanceService::unlock($data['code']);
        BalanceService::recount($this->unid);
        $this->success('确认收款成功');
    }
